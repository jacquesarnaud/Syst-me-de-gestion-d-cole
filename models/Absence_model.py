...
class AbsenceModels ( DatabaseManager ):
    def ajouter_absence(self, etudiant_id, matiere_id, date, status):
        self.cusor.execute('''INSERT INTO absences (etudiant_id, matiere_id, date, status) VALUES (?, ?, ?, ?)''', 
                            (etudiant_id, matiere_id, date, status))
        self.conn.commit()
    
    def lister_absences(self):
        self.cusor.execute('''SELECT a.id, e.nom, e.prenom, m.nom_matiere, a.date, a.status 
                            FROM absences a 
    ...
    def justifier_absence(self, absence_id):
        self.cusor.execute('''UPDATE absences SET status = 'justifiée' WHERE id = ?''', (absence_id,))
        self.conn.commit()

    def supprimer_absence(self, absence_id):
        self.cusor.execute('''DELETE FROM absences WHERE id = ?''', (absence_id,))
        self.conn.commit()
