...
class NotesModels ( DatabaseManager ):
    ...
    def ajouter_note(self, etudiant_id, matiere_id, note):
        self.cusor.execute('''INSERT INTO notes (etudiant_id, matiere_id, note) VALUES (?, ?, ?)''', 
                            (etudiant_id, matiere_id, note))
        self.conn.commit()
    
    def lister_notes(self):
        self.cusor.execute('''SELECT n.id, e.nom, e.prenom, m.nom_matiere, n.note 
                            FROM notes n 
    ...
    def modifier_note(self, note_id, nouvelle_note):
        self.cusor.execute('''UPDATE notes SET note = ? WHERE id = ?''', (nouvelle_note, note_id))
        self.conn.commit()

    def supprimer_note(self, note_id):
        self.cusor.execute('''DELETE FROM notes WHERE id = ?''', (note_id,))
        self.conn.commit()
